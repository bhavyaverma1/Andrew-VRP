    ...
    for node in range(0, len(data['time_matrix'])):
        if(node in data['starts'] or node in data['ends']):
            ...
            if data['pref_time_windows'][node]!=None:
                START_TIME_OF_DAY = int(os.getenv("START_TIME_OF_DAY")) # in minutes
                pref_start_time = data['pref_time_windows'][node][0]-START_TIME_OF_DAY
                pref_end_time = data['pref_time_windows'][node][1]-START_TIME_OF_DAY
#                 print(node,pref_start_time, pref_end_time)
    ...
    best_solution = None
    best_answer = -9999999
    NUM_SOLUTIONS = int(os.getenv("NUM_SOLUTIONS"))
    for i in range(NUM_SOLUTIONS):
        solution = routing.SolveWithParameters(search_parameters)
        if solution:
            total_distance,total_load,_ = get_distance_load_times(data, manager, routing, solution)
            TOTAL_LOAD_WEIGHTAGE = int(os.getenv("TOTAL_LOAD_WEIGHTAGE"))
            TOTAL_DISTANCE_WEIGHTAGE = int(os.getenv("TOTAL_DISTANCE_WEIGHTAGE"))
            curr_answer = TOTAL_LOAD_WEIGHTAGE * total_load - TOTAL_DISTANCE_WEIGHTAGE * total_distance # CHANGE THE WEIGHTS HERE
